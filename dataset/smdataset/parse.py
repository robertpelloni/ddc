...
ATTR_NAME_TO_PARSER = {
    ...
    "attacks": str_parser,
    "timesignatures": list_parser(
        kv_parser(float_parser, kv_parser(int_parser, int_parser))
    ),
        ...
        if attr_name in attrs:
            if attr_name in ATTR_MULTI:
                ...
                if attr_val_parsed != attrs[attr_name]:
                    parlog.warning(
                        "Attribute {} defined multiple times with different values".format(attr_name)
                    )
