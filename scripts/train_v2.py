...
def main():
    ...
    with open(os.path.join(args.dataset_dir, "train.txt"), "r", encoding="utf-8") as f:
        train_json_fps = [x.strip() for x in f.read().splitlines() if x.strip()]

    valid_json_fps = []
    if os.path.exists(os.path.join(args.dataset_dir, "valid.txt")):
        with open(os.path.join(args.dataset_dir, "valid.txt"), "r", encoding="utf-8") as f:
    ...
    if args.model_type == "onset":
        ...
        model = create_sym_model(seq_len, audio_shape, 0, vocab_size, config)
