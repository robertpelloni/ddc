...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Directory containing train.txt, valid.txt and json files",
    )
    parser.add_argument(
        "--feats_dir",
        type=str,
        required=True,
        help="Directory containing .npy audio features",
    )
    parser.add_argument(
        "--out_dir", type=str, required=True, help="Directory to save model checkpoints"
    )
    parser.add_argument(
...
<<<<<<< HEAD
    if args.model_type == "onset":
        train_gen = DataGenerator(
            train_json_fps, args.feats_dir, args.batch_size, "onset", config
        )
        valid_gen = (
            DataGenerator(
                valid_json_fps, args.feats_dir, args.batch_size, "onset", config
            )
            if valid_json_fps
            else None
        )
...
<<<<<<< HEAD
        ...
        # Enforce sequential execution if needed by avoiding multiprocessing in fit generator
        # use_multiprocessing=False, workers=1 is default for newer TF if input is a python generator
        model.fit(
            train_gen,
            ...
            callbacks=[
                tf.keras.callbacks.ModelCheckpoint(
                    os.path.join(args.out_dir, "model_{epoch:02d}.h5")
                )
            ],
        )
...
<<<<<<< HEAD
        ...
        train_gen = DataGenerator(
            train_json_fps, args.feats_dir, args.batch_size, "sym", config, vocab_map
        )
        valid_gen = (
            DataGenerator(
                valid_json_fps,
