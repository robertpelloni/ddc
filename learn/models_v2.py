    ...
    def __init__(self, audio_shape, n_other_features, config):
        ...
        for i, size in enumerate(dnn_sizes):
            self.dnn_layers.append(
                layers.Dense(size, activation=config.get("dnn_nonlin", "relu"), name=f"dnn_{i}")
            )
...
class SymNetV2(models.Model):
    def __init__(self, audio_shape, n_other_features, vocab_size, config):
        ...
        for shape, pool in zip(cnn_filter_shapes, cnn_pool):
            conv = layers.Conv2D(
                filters=shape[2],
                kernel_size=(shape[0], shape[1]),
                activation="relu",
        ...
        if config.get("cnn_dim_reduction_size", -1) > 0:
            self.cnn_dim_redux = layers.Dense(config["cnn_dim_reduction_size"], activation="relu")

        self.rnn_size = config.get("rnn_size", 128)
        self.rnn_layers = []
        n_layers = config.get("rnn_nlayers", 1)
        for _ in range(n_layers):
            self.rnn_layers.append(layers.LSTM(self.rnn_size, return_sequences=True, return_state=False))
...
def create_onset_model(input_shape, n_other, config):
    ...
    for shape, pool in zip(cnn_filter_shapes, cnn_pool):
        x = layers.Conv2D(filters=shape[2], kernel_size=(shape[0], shape[1]), activation="relu", padding="same")(x)
    ...
    return models.Model(inputs=[audio_in, other_in], outputs=outputs, name="OnsetNet")
...
def create_sym_model(seq_len, audio_shape, n_other, vocab_size, config):
    ...
    for shape, pool in zip(cnn_filter_shapes, cnn_pool):
        conv = layers.Conv2D(filters=shape[2], kernel_size=(shape[0], shape[1]), activation="relu", padding="same")
    ...
    if config.get("cnn_dim_reduction_size", -1) > 0:
        x_audio = layers.TimeDistributed(layers.Dense(config["cnn_dim_reduction_size"], activation="relu"))(x_audio)
    ...
    return models.Model(inputs=[sym_in, audio_in, other_in], outputs=outputs, name="SymNet")
