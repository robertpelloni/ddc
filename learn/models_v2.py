...
=======
        ...
        for i, (shape, pool) in enumerate(zip(cnn_filter_shapes, cnn_pool)):
            self.cnn_layers.append(layers.Conv2D(
                filters=shape[2],
                kernel_size=(shape[0], shape[1]),
                activation='relu',
                padding='same',
            ...
            self.cnn_layers.append(layers.MaxPool2D(
                pool_size=pool,
                strides=pool,
                padding='same',
                name=f'pool_{i}'
        ...
        dnn_sizes = config.get('dnn_sizes', [])
        for i, size in enumerate(dnn_sizes):
            self.dnn_layers.append(layers.Dense(size, activation=config.get('dnn_nonlin', 'relu'), name=f'dnn_{i}'))
            if config.get('dnn_keep_prob', 1.0) < 1.0:
                self.dnn_layers.append(layers.Dropout(1.0 - config['dnn_keep_prob']))
...
=======
    ...
    model = models.Model(inputs=[sym_in, audio_in, other_in], outputs=outputs, name='SymNet')
>>>>>>> origin/ddc-modernization-and-integration-14116118131799338522
    return model
