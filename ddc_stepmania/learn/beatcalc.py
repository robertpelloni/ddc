...
_EPSILON = 1e-6
<<<<<<< HEAD:ddc_stepmania/learn/beatcalc.py
    ...
    def __init__(self, offset, beat_bpm, beat_stop):
        ...
        for beat, stop in beat_stop:
            ...
            if beat in stops:
                stops[beat] += stop
            stops[beat] = stop
        beat_stop = list(filter(lambda x: x[1] != 0.0, sorted(stops.items(), key=lambda x: x[0])))
=======
...
>>>>>>> origin/master_v2:ddc/beattimecalc.py
