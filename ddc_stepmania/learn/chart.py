<<<<<<< HEAD:ddc_stepmania/learn/chart.py
...
from ddc_stepmania.learn.beatcalc import BeatCalc
from ddc_stepmania.learn.common_util import make_onset_feature_context, np_pad
...
class Chart(object):
    def __init__(self, song_metadata, metadata, annotations):
        ...
        self.first_annotation_time = self.annotations[0][1]
        self.last_annotation_time = self.annotations[-1][1]
...
class OnsetChart(Chart):
    def __init__(self, song_metadata, song_features, frame_rate, metadata, annotations):
        ...
        if song_features is not None:
            ...
            song_fpath = self.song_metadata['fpath']
            if song_fpath and os.path.exists(song_fpath):
                try:
                    duration = librosa.get_duration(filename=song_fpath)
                    self.nframes = int(round(duration * frame_rate))
                except Exception as e:
                    print(f"Warning: could not process {song_fpath} with librosa: {e}")
                    self.nframes = 0
            else:
                self.nframes = 0
            ...
            if self.nframes == 0:
                if annotations:
                    last_note_time = annotations[-1][1]
                    duration = last_note_time + 5.0
                    self.nframes = int(round(duration * frame_rate))
                else:
                    self.nframes = 0

        self.dt = dt = 1.0 / frame_rate
        self.onsets = set(int(round(t / dt)) for _, t, _, _ in self.annotations)
    ...
    def get_example(self,
        ...
        if self.song_features is None:
            ...
            self.song_features = self._feature_extractor.extract_features(self.song_metadata['fpath'])
