...
from ddc_stepmania.learn.onset_net import OnsetNet
from ddc_stepmania.learn.util import *

# Data
flags.DEFINE_string('train_txt_fp', '', 'Training dataset txt file with a list of pickled song files')
flags.DEFINE_string('valid_txt_fp', '', 'Eval dataset txt file with a list of pickled song files')
flags.DEFINE_bool('z_score', False, 'If true, train and test on z-score of training data')
flags.DEFINE_string('test_txt_fp', '', 'Test dataset txt file with a list of pickled song files')
flags.DEFINE_string('model_ckpt_fp', '', 'File path to model checkpoint if resuming or eval')
flags.DEFINE_string('export_feat_name', '', 'If set, export CNN features to this directory')

# Features
flags.DEFINE_integer('audio_context_radius', 7, 'Past and future context per training example')
flags.DEFINE_integer('audio_nbands', 80, 'Number of bands per frame')
flags.DEFINE_integer('audio_nchannels', 3, 'Number of channels per frame')
flags.DEFINE_string('audio_select_channels', '', 'List of CSV audio channels. If non-empty, other channels excluded from model.')
flags.DEFINE_string('feat_diff_feet_to_id_fp', '', '')
flags.DEFINE_string('feat_diff_coarse_to_id_fp', '', '')
flags.DEFINE_bool('feat_diff_dipstick', False, '')
flags.DEFINE_string('feat_freetext_to_id_fp', '', '')
flags.DEFINE_bool('feat_beat_phase', False, '')
flags.DEFINE_bool('feat_beat_phase_cos', False, '')

# Network params
flags.DEFINE_string('cnn_filter_shapes', '', 'CSV 3-tuples of filter shapes (time, freq, n)')
flags.DEFINE_string('cnn_pool', '', 'CSV 2-tuples of pool amounts (time, freq)')
flags.DEFINE_bool('cnn_rnn_zack', False, '')
flags.DEFINE_integer('zack_hack', 0, '')
flags.DEFINE_string('rnn_cell_type', 'lstm', '')
flags.DEFINE_integer('rnn_size', 0, '')
flags.DEFINE_integer('rnn_nlayers', 0, '')
flags.DEFINE_integer('rnn_nunroll', 1, '')
flags.DEFINE_float('rnn_keep_prob', 1.0, '')
flags.DEFINE_string('dnn_sizes', '', 'CSV sizes for dense layers')
flags.DEFINE_float('dnn_keep_prob', 1.0, '')
flags.DEFINE_string('dnn_nonlin', 'sigmoid', '')

# Training params
flags.DEFINE_integer('batch_size', 256, 'Batch size for training')
flags.DEFINE_string('weight_strategy', 'rect', 'One of \'rect\' or \'last\'')
flags.DEFINE_bool('randomize_charts', False, '')
flags.DEFINE_bool('balanced_class', True, 'If true, balance classes, otherwise use prior.')
flags.DEFINE_integer('exclude_onset_neighbors', 0, 'If nonzero, excludes radius around true onsets from dataset as they may be misleading true neg.')
flags.DEFINE_bool('exclude_pre_onsets', False, 'If true, exclude all true neg before first onset.')
flags.DEFINE_bool('exclude_post_onsets', False, 'If true, exclude all true neg after last onset.')
flags.DEFINE_float('grad_clip', 0.0, 'Clip gradients to this value if greater than 0')
flags.DEFINE_string('opt', 'sgd', 'One of \'sgd\'')
flags.DEFINE_float('lr', 1.0, 'Learning rate')
flags.DEFINE_float('lr_decay_rate', 1.0, 'Multiply learning rate by this value every epoch')
flags.DEFINE_integer('lr_decay_delay', 0, '')
flags.DEFINE_integer('nbatches_per_ckpt', 100, 'Save model weights every N batches')
flags.DEFINE_integer('nbatches_per_eval', 10000, 'Evaluate model every N batches')
flags.DEFINE_integer('nepochs', 0, 'Number of training epochs, negative means train continuously')
flags.DEFINE_string('experiment_dir', '', 'Directory for temporary training files and model weights')

# Eval params
flags.DEFINE_string('eval_window_type', '', '')
flags.DEFINE_integer('eval_window_width', 0, '')
flags.DEFINE_integer('eval_align_tolerance', 2, '')
flags.DEFINE_string('eval_charts_export', '', '')
flags.DEFINE_string('eval_diff_coarse', '', '')
...
def main(_):
    ...
    model_config = {
        ...
        'cnn_init': variance_scaling_initializer(factor=1.43, uniform=True, dtype=dtype),
        ...
        'rnn_init': tf.random_uniform_initializer(-5e-2, 5e-2),
        'rnn_nunroll': FLAGS.rnn_nunroll,
        'rnn_keep_prob': FLAGS.rnn_keep_prob,
        'dnn_sizes': stride_csv_arg_list(FLAGS.dnn_sizes, 1, int),
        'dnn_init': variance_scaling_initializer(factor=1.15, uniform=True, dtype=dtype),
    ...
    with tf.Graph().as_default(), tf.compat.v1.Session() as sess:
        ...
        # Create summaries
        if do_train:
            summary_writer = tf.compat.v1.summary.FileWriter(FLAGS.experiment_dir, sess.graph)
            ...
            while FLAGS.nepochs < 0 or batch_num < nbatches:
                ...
                if do_train_eval and batch_num % FLAGS.nbatches_per_eval == 0:
                    ...
                    if auprc_mean > eval_best_auprc:
                        ...
                        model_early_stop_auprc.save(sess, ckpt_fp, global_step=tf.compat.v1.train.get_or_create_global_step())
                    ...
                    if fscore_mean > eval_best_fscore:
                        ...
                        model_early_stop_fscore.save(sess, ckpt_fp, global_step=tf.compat.v1.train.get_or_create_global_step())
